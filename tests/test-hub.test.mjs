...
// test hooks: URL params (?time/?wx/?loader/?loaderhold/?ldhole/?yorb/?playshead/?playstail/?wind/?aim/?impact),
// live console helpers (setTime/setWx/setWind/setAim/setImpact/toggleCompass) + loader power-up form classes.
...
export const tests = {
  ...
  'app still exposes the live console helpers the hub drives'(){
    const i = readIndex();
    for(const tok of ['function setTime', 'window.setWx=', 'toggleCompass', 'window.setWind=', 'window.setAim =', 'window.setImpact ='])
  ...
  'hub still sends the loader/plays hooks it documents'(){
    ...
    for(const tok of ['loader=', 'loaderhold=1', 'ldhole=hole', 'yorb=', 'playshead', 'playstail',
                      "'wind='", 'setWind', "'aim='", 'setAim', "'impact='", 'setImpact'])
